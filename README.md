...
[![Proven on Windows Server 2019 and Windows 11](https://img.shields.io/badge/proven%20on-Server%202019%20%2B%20Windows%2011-146EFA?style=flat-square&labelColor=373637)](docs/VALIDATION.md)
...
| `anti-tampering/` | Protect the recorder — stop an attacker or a careless tech from switching it back off | 9 |
...
**Running v1.0.0 on Windows 10/11 workstations?** Two of its alerts there were
false — a PowerShell 2.0 bypass that does not exist on current builds, and a
`2` exit from `Enable-VssPreservation` for a capability client Windows does not
...
Every script that changes anything has three modes. `Test-VisibilityDrift` and
`Test-DefenderPosture` are read-only by design and have `-Audit` only:
