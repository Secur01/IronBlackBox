<#
.SYNOPSIS
    Protects the event logs a responder depends on: reports who can clear them,
    optionally removes that right, and configures retention so a full log
    archives instead of losing events.

.DESCRIPTION
    Enable-IRVisibility fills the event logs. This script keeps them.

    Three concerns:

    1. WHO CAN CLEAR A LOG. Each channel carries a security descriptor. On
       Windows Server 2019 the Security channel grants BUILTIN\Administrators
       mask 0x5 - read (0x1) plus CLEAR (0x4) - so any administrator, or
       anything running as one, can erase it. This script decodes that SDDL and
       says so in plain terms. It removes the clear right only when
       -RemoveAdminClear is passed, because taking it away breaks any legitimate
       tool that archives-then-clears, and an administrator can put it back by
       rewriting the descriptor anyway. The value is friction and evidence, not
       prevention.

    2. WHAT HAPPENS WHEN A LOG FILLS UP. The Retention policy is dangerous and
       counter-intuitive. Microsoft's own help text: enabled and full means "new
       events are not written to the log and are lost" - the log STOPS
       RECORDING. AutoBackupLogFiles only takes effect when Retention is
       enabled, and turns a full log into a renamed archive plus a fresh file.
       So Retention alone is the worst setting available, and Retention plus
       AutoBackup is the one that loses nothing. Both are REG_SZ, not DWORD -
       the ADMX declares <string>1</string>.

    3. WHETHER ARCHIVING IS ABOUT TO FILL THE DISK. Archives accumulate with no
       automatic purge. This script reports free space and the archive footprint
       as a finding rather than deleting anything: see the note below.

    NOT IMPLEMENTED, deliberately: automatic purging of old archives. Doing that
    safely needs the exact file-naming pattern Windows uses for AutoBackup
    archives, and it could not be observed on the lab - the MaxSize registry
    policy overrides the local channel config, so a rollover cannot be forced by
    shrinking the channel. Code that DELETES .evtx files on a guessed pattern is
    how evidence gets destroyed, so it is not written until the pattern is
    observed. Until then this script warns and the operator decides.

.PARAMETER Audit
    Default. Strictly read-only. Decodes each channel's descriptor, reports who
    can clear it, and reports retention and disk headroom.

.PARAMETER Apply
    Applies the retention configuration, recording previous values first.
    Touches the security descriptor only with -RemoveAdminClear.

.PARAMETER Rollback
    Restores what a prior -Apply changed, descriptors included.

.PARAMETER ToolkitRoot
    Base directory for the manifest. Default C:\ProgramData\IronBlackBox.

.PARAMETER Channels
    Channels to act on. Default Security, Application, System.

.PARAMETER RemoveAdminClear
    Also remove the CLEAR right from BUILTIN\Administrators on each channel.
    Off by default - read the DESCRIPTION before using it.

.PARAMETER MinimumFreeDiskPercent
    Report a finding when a volume holding one of the -Channels log files has
    less than this percentage free. Applied per volume, and the volume is read
    from each channel rather than assumed to be the system one: a relocated
    channel archives somewhere else. Default 15.
...
.EXAMPLE
    .\Protect-EventLogs.ps1
    Reports who can clear each log, how it is configured, and disk headroom.

.EXAMPLE
    .\Protect-EventLogs.ps1 -Apply
    Configures retention so a full log archives rather than losing events.

.EXAMPLE
    .\Protect-EventLogs.ps1 -Apply -RemoveAdminClear
    Also removes the administrators' right to clear the logs.
...
param(
    ...
    [Parameter()]
    [string[]] $Channels = @('Security', 'Application', 'System'),

    [Parameter()]
    [switch] $RemoveAdminClear,
...
$script:ScriptName    = 'Protect-EventLogs'
...
#   Retention and AutoBackupLogFiles per channel, nothing else.
...
<#
    Channel access rights. Verified by decoding the live descriptors on the lab
    and cross-reading them against what each principal plausibly needs:

      0x1  read the log
      0x2  write to the log
      0x4  CLEAR the log
      0xF0000  the standard rights (delete, read/write DAC, write owner)

    Server 2019 Security channel, as shipped:
      SYSTEM                    0xF0005  standard + clear + read
      BUILTIN\Administrators    0x5      clear + read
      BUILTIN\Event Log Readers 0x1      read

    So an administrator can erase the Security log, and does not need
    WRITE_DAC to do it.
#>
...
function Get-ChannelSddl {
    ...
    if ($result.ExitCode -ne 0) {
        throw ('wevtutil gl failed for channel ' + $Channel + ': ' + ($result.Output -join ' '))
    }
    foreach ($line in $result.Output) {
        if ($line -match '^\s*channelAccess:\s*(.+)$') {
            return $Matches[1].Trim()
        }
    }
    throw ('wevtutil gl returned no channelAccess for channel ' + $Channel)
}
...
# Principals that cannot legitimately hold write or CLEAR on an event log. If a
# manifest record asks -Rollback to grant one of these, the record is wrong or
# planted; a host whose genuine pre-apply descriptor looked like this has a
# problem an operator needs to see, not one a rollback should quietly recreate.
# https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/understand-security-identifiers
...
function Assert-ChannelRestorable {
    <#
        THE CONSTRAINT ON A MANIFEST-SUPPLIED CHANNEL AND DESCRIPTOR.

        -Rollback used to take both straight from the record and hand them to
        'wevtutil sl <channel> /ca:<sddl>' as SYSTEM. Assert-SddlNotMorePermissive
        guards the APPLY path and cannot guard this one: a rollback legitimately
        LOOSENS a descriptor, because putting back the pre-apply state is by
        definition granting back what the apply removed. So it was left out
        entirely, and the restore path had no guard at all.

        docs/DESIGN.md section 4: "The manifest is operator-writable input, not
        trusted state." Enable-IRVisibility's Restore-EventChannelChange already
        refuses a channel that script does not own, for exactly this reason. This
        is the same constraint, owned by the script rather than supplied by the
        record:

          1. the channel must be one -Channels names, and must exist on this host
          2. the descriptor must parse as a security descriptor
          3. it must not grant write or CLEAR to a well-known unprivileged
             principal - the one thing a pre-apply event-log descriptor cannot
             legitimately have said

        Honest about the ceiling: with the toolkit root now ACL'd to SYSTEM and
        Administrators, anyone who can edit a manifest record is already an
        administrator. This is defence in depth and a guard against a hand-edited
        record or a bad merge. The DACL is what makes the manifest trustworthy.

        Returns $true when the restore may proceed; otherwise writes the finding
        and returns $false.
    #>
    param(
        [Parameter(Mandatory = $true)][string] $Channel,
        [Parameter(Mandatory = $true)][AllowEmptyString()][string] $Sddl,
        [Parameter(Mandatory = $true)][string[]] $OwnedChannel
    )

    if ([string]::IsNullOrWhiteSpace($Channel)) {
        Write-Finding 'The manifest record names no channel; leaving it alone.'
        return $false
    }
    $owned = $false
    foreach ($candidate in $OwnedChannel) {
        if ([string]::Equals($candidate, $Channel, [System.StringComparison]::OrdinalIgnoreCase)) {
            $owned = $true; break
        }
    }
    if (-not $owned) {
        Write-Finding ('The manifest names channel "' + $Channel + '", which this run does not own (' +
                       ($OwnedChannel -join ', ') + '). Refusing to reconfigure it - pass -Channels to ' +
                       'include it if that is really intended.')
        return $false
    }
    $probe = Invoke-NativeCommand -FilePath $script:WevtutilPath -Arguments @('gl', $Channel)
    if ($probe.ExitCode -ne 0) {
        Write-Finding ('Channel "' + $Channel + '" cannot be read on this host, so its descriptor is ' +
                       'not being written: ' + ($probe.Output -join ' '))
        return $false
    }

    if ([string]::IsNullOrWhiteSpace($Sddl)) {
        Write-Finding ('The manifest records no previous descriptor for ' + $Channel +
                       '; leaving it alone.')
        return $false
    }
    $map = $null
    try { $map = Get-SddlAceMap -Sddl $Sddl }
    catch {
        Write-Finding ('The recorded descriptor for ' + $Channel + ' does not parse as a security ' +
                       'descriptor, so it is not being written: ' + $_.Exception.Message)
        return $false
    }

    $dangerous = ($script:RightWrite -bor $script:RightClear)
    foreach ($sid in $map.Keys) {
        if ([string] $sid -like 'DENY:*') { continue }
        if ($script:UnprivilegedSids -notcontains [string] $sid) { continue }
        $granted = $map[$sid] -band $dangerous
        if ($granted -ne 0) {
            Write-Finding ('REFUSING to restore the descriptor on ' + $Channel + ': it would grant ' +
                           (Get-SidDisplayName -Sid ([string] $sid)) + ' ' +
                           (Format-RightMask -Mask $granted) + ' on an event log. A rollback does not ' +
                           'get to hand out the ability to erase evidence, whatever the record says.')
            return $false
        }
    }
    return $true
...
function Set-TrackedChannelSddl {
    <#
        Removes the clear right from Administrators on one channel. Records the
        previous descriptor first, verifies the result is strictly less
        permissive, writes it, then re-reads and confirms the right is actually
        gone.

        That last step is not ceremony. On the lab,
        DiscretionaryAcl.RemoveAccess silently left the mask untouched while
        happily regenerating a different-looking descriptor - so "the API
        returned without error" proves nothing about whether the right was
        removed.
    #>
    param([Parameter(Mandatory = $true)][string] $Channel)

    $before = Get-ChannelSddl -Channel $Channel
    $beforeMap = Get-SddlAceMap -Sddl $before

    if (-not $beforeMap.ContainsKey($script:AdminsSid)) {
        Write-Ok ($Channel + ': administrators have no explicit ACE; nothing to remove')
        return $false
    }
    if (($beforeMap[$script:AdminsSid] -band $script:RightClear) -eq 0) {
        Write-Ok ($Channel + ': administrators already cannot clear this log')
        return $false
    }

    if (-not $Apply) {
        Write-Finding ($Channel + ': would remove the CLEAR right from administrators')
        return $false
    }

    # Build the target descriptor by clearing the bit explicitly, rather than
    # trusting an ACL helper to do it.
    $sd = New-Object System.Security.AccessControl.CommonSecurityDescriptor($false, $false, $before)
    $adminsSid = New-Object System.Security.Principal.SecurityIdentifier($script:AdminsSid)
    $target = $beforeMap[$script:AdminsSid] -band (-bnot $script:RightClear)
    ...
    Assert-SddlNotMorePermissive -Before $before -After $after -Channel $Channel

    $afterMap = Get-SddlAceMap -Sddl $after
    if (($afterMap[$script:AdminsSid] -band $script:RightClear) -ne 0) {
        throw ('Building the descriptor for ' + $Channel +
               ' did not actually remove the clear right; refusing to write it.')
    }
    ...
    [void] (Write-ManifestChange -Change @{
        ...
        previousSddl  = $before
        newSddl       = $after
        description   = ($Channel + ': removed the CLEAR right from administrators')
    })
    ...
    $confirmMap = Get-SddlAceMap -Sddl (Get-ChannelSddl -Channel $Channel)
    if ($confirmMap.ContainsKey($script:AdminsSid) -and
        ($confirmMap[$script:AdminsSid] -band $script:RightClear) -ne 0) {
        throw ('Descriptor written for ' + $Channel + ' but administrators can still clear it.')
    }

    Write-Ok ($Channel + ': administrators can no longer clear this log')
    return $true
}

function Restore-ChannelAccess {
    param([Parameter(Mandatory = $true)] $ChangeRecord)

    $change = $ChangeRecord.change
    $current = Get-ChannelSddl -Channel $change.channel
    $currentMap = Get-SddlAceMap -Sddl $current
    $intendedMap = Get-SddlAceMap -Sddl $change.newSddl

    # Compare by mask, not by text: the descriptor is stored in whatever form
    # .NET generated, and Windows may print it differently.
    ...
    if (-not $same) {
        # CASE 2 of the rollback doctrine (see the Registry region): does the
        # channel already hold the descriptor recorded BEFORE this run? Either
        # this rollback already ran, or the write never landed - and both mean
        # there is nothing to do and nothing wrong.
        #
        # Without this, a second -Rollback declined all three channels with "the
        # descriptor no longer matches what this run set" about descriptors the
        # toolkit itself had just restored. The run then never left the eligible
        # set and Test-VisibilityDrift reported every change in it as drifted.
        # That was R-1, measured on the domain member: 29 false drifts, 9 of them
        # from this script.
        $previousMap = Get-SddlAceMap -Sddl $change.previousSddl
        ...
        if ($alreadyPrevious) {
            Write-Ok ($change.channel +
                      ' already holds the descriptor recorded before this run; nothing to restore.')
            return $script:RollbackRestored
        }

        Write-Finding ($change.channel +
                       ': the descriptor holds neither what this run set nor what was recorded before it;' +
                       ' leaving it alone.')
        return 'declined'
    }

    if (-not (Assert-ChannelRestorable -Channel ([string] $change.channel) `
                -Sddl ([string] $change.previousSddl) -OwnedChannel $Channels)) {
        return 'declined'
    }

    $result = Invoke-NativeCommand -FilePath $script:WevtutilPath -Arguments @(
        'sl', $change.channel, ('/ca:' + $change.previousSddl))
    if ($result.ExitCode -ne 0) {
        throw ('wevtutil sl failed restoring ' + $change.channel + ': ' + ($result.Output -join ' '))
    }

    # READ IT BACK. This used to return 'restored' on wevtutil's exit code alone,
    # and then the run was marked completed - which takes it out of the eligible
    # set and out of what Test-VisibilityDrift expects. docs/AUTHORING.md: "a native
    # tool's exit code is not a claim about its output". A descriptor reported as
    # restored and not actually restored is the worst of both: the evidence is
    # unprotected and nothing is watching for it any more.
    $after = $null
    try { $after = Get-ChannelSddl -Channel ([string] $change.channel) }
    catch {
        Write-Finding ('wevtutil accepted the descriptor for ' + $change.channel +
                       ' but it cannot be read back, so the restore is not confirmed: ' +
                       $_.Exception.Message)
        return 'declined'
    }
    if (-not [string]::Equals(([string] $after).Trim(), ([string] $change.previousSddl).Trim(),
                              [System.StringComparison]::OrdinalIgnoreCase)) {
        Write-Finding ('wevtutil exited 0 restoring ' + $change.channel + ' but the channel now reads ' +
                       $after + ', not the recorded ' + [string] $change.previousSddl + '.')
        return 'declined'
    }
    Write-Ok ('Restored the descriptor on ' + $change.channel + ' (read back and confirmed)')
    return 'restored'
}

#endregion

#region Retention and headroom ------------------------------------------------

# Both are REG_SZ, not DWord: EventLog.admx declares <string>1</string> for the
# enabled value of each. Writing a DWord here writes the wrong type.
$script:KeyEventLogBase = 'HKLM:\Software\Policies\Microsoft\Windows\EventLog'

function Test-ChannelIsClassic {
    <#
        Whether the EventLog POLICY can reach this channel at all.

        Retention and AutoBackupLogFiles are written under
        HKLM:\Software\Policies\Microsoft\Windows\EventLog\<channel>, and that
        policy only governs CLASSIC logs. Point it at a modern channel and the
        values are written, the registry reads them back, and the channel's actual
        retention does not move.

        MEASURED, which is why this guard exists: -Channels
        'Microsoft-Windows-SmbClient/Security' -Apply exited 0, printed
        "archive the log when it fills up - set" and "retain old events - set",
        reported "2 change(s) applied", created the policy key with
        AutoBackupLogFiles=1 and Retention=1 - and 'wevtutil gl' reported
        retention:false both before and after. The script claimed to protect a log
        it had not protected, which is the one thing docs/AUTHORING.md says it may never
        do.

        Get-WinEvent -ListLog exposes IsClassicLog, so this is a property of the
        channel rather than a guess from its name. Measured on Server 2019:
        Security, Application, System and 'Windows PowerShell' are True;
        Microsoft-Windows-SmbClient/Security and
        Microsoft-Windows-PowerShell/Operational are False.

        Returns $true only when the channel is classic and can therefore be
        governed. Fails CLOSED: a channel whose type cannot be read is not
        treated as classic.
    #>
    param([Parameter(Mandatory = $true)][string] $Channel)
    ...
    if ($null -eq $log.PSObject.Properties['IsClassicLog']) { return $false }
    if ($null -eq $log.IsClassicLog) { return $false }
    return [bool] $log.IsClassicLog
}

function Set-ChannelRetention {
    <#
        Retention=1 with AutoBackupLogFiles=1 is the only combination that
        neither stops recording nor discards old events. Microsoft's help text
        for Retention, verbatim: "If you enable this policy setting and a log
        file reaches its maximum size, new events are not written to the log and
        are lost." AutoBackup is what turns that into "archive and start a new
        file" instead.

        The pair is written AutoBackup first, then Retention. Order matters: the
        dangerous state is Retention on with AutoBackup off, and writing them
        the other way round would pass through it.
    #>
    param([Parameter(Mandatory = $true)][string] $Channel)

    $key = ($script:KeyEventLogBase + '\' + $Channel)
    $changes = 0

    if (Set-TrackedRegistryValue -Path $key -Name 'AutoBackupLogFiles' `
        -Kind 'String' -Value '1' `
        -Description ($Channel + ': archive the log when it fills up')) { $changes++ }

    if (Set-TrackedRegistryValue -Path $key -Name 'Retention' `
        -Kind 'String' -Value '1' `
        -Description ($Channel + ': retain old events (archived, not discarded)')) { $changes++ }

    return $changes
}

function Get-ChannelLogFile {
    <#
        Where a channel's .evtx actually lives, READ rather than assumed.

        A channel can be relocated to another volume. Measuring
        %SystemRoot%\System32\winevt\Logs then answers about a volume that will
        receive none of that channel's archives - the same shape of defect
        Enable-VssPreservation carried as V-1, and the reason
        Enable-IRVisibility reads this same line in Get-EventChannelFileState.

        verification/facts.json, 'wevtutil-gl-reports-logfilename': 'wevtutil gl
        <channel>' emits, indented under a 'logging:' heading,
        'logFileName: %SystemRoot%\System32\Winevt\Logs\Security.evtx', with the
        environment variable left UNEXPANDED - so the value is expanded before
        it touches the filesystem.

        A second 'wevtutil gl' per channel on top of Get-ChannelSddl's,
        deliberately: one parser per concern beats one function returning two
        unrelated things, and this runs once per channel per run.

        # UNVERIFIED: whether the 'logFileName' key name is localised on a
        # non-English host. If it were, the match fails and this returns $null,
        # and the caller reports the channel as unmeasured rather than falling
        # back to a guessed path - guessing is exactly how the wrong volume gets
        # measured.
    #>
    param([Parameter(Mandatory = $true)][string] $Channel)
...
function Test-EventLogHeadroom {
    <#
        Archives accumulate and nothing purges them. Reported, never acted on:
        this script does not delete .evtx files. See the note in the script
        header for why the purge is not implemented yet.

        MEASURED PER VOLUME, and the volume is read from each channel rather
        than assumed to be the system one. An AutoBackup archive is a rename of
        the log file (verification/facts.json, 'eventlog-retention-is-reg-sz':
        AutoBackupLogFiles "turns a full log into a renamed archive plus a fresh
        file"), so it lands in the log file's own directory - and a channel
        whose log file was relocated archives onto a volume that a hard-coded
        %SystemRoot% path never measures. Printing 'enough headroom' about the
        wrong volume is a false reassurance about the only number this section
        exists to produce.
    #>
    Write-Section 'Disk headroom for archived logs'

    $resolved = New-Object System.Collections.ArrayList
    foreach ($channel in $Channels) {
        $logFile = Get-ChannelLogFile -Channel $channel
        if ($null -eq $logFile) {
            # No -Apply of this script can make wevtutil report a path it did
            # not report, so this is a host limit, not a finding.
            Write-HostLimit ($channel + ': could not read where its log file lives, so the volume ' +
                             'that would hold its archives was not measured.')
            continue
        }
        [void] $resolved.Add([PSCustomObject] @{
            Channel    = $channel
            Directory  = $logFile.Directory
            VolumeRoot = $logFile.VolumeRoot
        })
    }

    if ($resolved.Count -eq 0) {
        Write-Info 'No channel reported a log file path, so there was no volume to measure.'
        return
    }

    $volumes = @($resolved | ForEach-Object { $_.VolumeRoot } | Sort-Object -Unique)
    foreach ($volume in $volumes) {
        $onVolume     = @($resolved | Where-Object { $_.VolumeRoot -eq $volume })
        $channelNames = @($onVolume | ForEach-Object { $_.Channel })
        $directories  = @($onVolume | ForEach-Object { $_.Directory } | Sort-Object -Unique)

        $drive = $null
        try { $drive = New-Object System.IO.DriveInfo($volume) }
        catch { $drive = $null }
        # IsReady before TotalSize: reading the size of a drive that is not ready
        # throws. https://learn.microsoft.com/en-us/dotnet/api/system.io.driveinfo.isready
        if ($null -eq $drive -or -not $drive.IsReady) {
            Write-HostLimit ('volume ' + $volume + ' holds the log file(s) for ' +
                             ($channelNames -join ', ') + ' and its free space could not be read.')
            continue
        }

        $freePercent = 0
        if ($drive.TotalSize -gt 0) {
            $freePercent = [int](($drive.AvailableFreeSpace / $drive.TotalSize) * 100)
        }
        ...
        foreach ($directory in $directories) {
            if (-not (Test-Path -LiteralPath $directory)) {
                Write-Finding ('Event log directory not found: ' + $directory)
                continue
            }
            $evtx = @(Get-ChildItem -LiteralPath $directory -Filter '*.evtx' -ErrorAction SilentlyContinue)
            $evtxCount += $evtx.Count
            if ($evtx.Count -gt 0) {
                $totalBytes += [decimal] (($evtx | Measure-Object -Property Length -Sum).Sum)
            }
        }
        $totalMb = [math]::Round($totalBytes / 1MB, 1)

        # [string] casts are load-bearing: in PowerShell, Int32 + String coerces
        # the STRING to Int32 and throws. An expression whose left operand is a
        # number must not start a concatenation.
        Write-Info ('volume ' + $volume + ' holds the log file(s) for ' + ($channelNames -join ', '))
        Write-Info ('  ' + [string] $evtxCount + ' .evtx file(s) using ' + [string] $totalMb +
                    ' MB in ' + ($directories -join ', '))
        Write-Info ('  ' + [string] $freeGb + ' GB free (' + [string] $freePercent + '%)')

        if ($freePercent -lt $MinimumFreeDiskPercent) {
            Write-Finding ('Only ' + [string] $freePercent + '% free on ' + $volume +
                           ', which holds the log file(s) for ' + ($channelNames -join ', ') +
                           ' - archived logs accumulate and are not purged automatically. ' +
                           'Review and archive them off this host.')
        }
        else {
            Write-Ok ('enough headroom for archiving on ' + $volume)
        }
    }
}

function Invoke-HostCheck {
    Write-Section 'Who can clear each log'

    foreach ($channel in $Channels) {
        $sddl = Get-ChannelSddl -Channel $channel
        $map = Get-SddlAceMap -Sddl $sddl
        $clearers = @()
        foreach ($sid in $map.Keys) {
            if ($sid -like 'DENY:*') { continue }
            if (($map[$sid] -band $script:RightClear) -ne 0) {
                $clearers += (Get-SidDisplayName -Sid $sid)
            }
        }
        if ($clearers.Count -eq 0) {
            Write-Ok ($channel + ': nothing can clear this log')
        }
        else {
            Write-Info ($channel + ': can be cleared by ' + ($clearers -join ', '))
        }
        if ($map.ContainsKey($script:AdminsSid)) {
            Write-Info ('  administrators hold ' + (Format-RightMask -Mask $map[$script:AdminsSid]))
        }
    }
    ...
    if ($RemoveAdminClear) {
        Write-Section 'Removing the administrators CLEAR right'
        foreach ($channel in $Channels) {
            if (Set-TrackedChannelSddl -Channel $channel) { $changeCount++ }
        }
    }
    else {
        Write-Info ''
        Write-Info 'Not changing any descriptor. Re-run with -RemoveAdminClear to remove'
        Write-Info 'the administrators CLEAR right - read the script header first.'
    }

    Write-Section 'Retention: archive instead of losing events'
    foreach ($channel in $Channels) {
        if (-not (Test-ChannelIsClassic -Channel $channel)) {
            Write-Finding ($channel + ' is not a classic log, so the EventLog retention policy cannot ' +
                           'reach it - writing the values would report success and change nothing. Size ' +
                           'a modern channel instead: Enable-IRVisibility does it through ' +
                           '"wevtutil sl /ms:", which is the only mechanism that works on one.')
            continue
        }
        $changeCount += Set-ChannelRetention -Channel $channel
    }

    Test-EventLogHeadroom
    return $changeCount
}
...
function Invoke-Main {
    ...
    Assert-ParameterNotEmpty -Name 'Channels' -Value $Channels
    Assert-ParameterRange   -Name 'MinimumFreeDiskPercent' -Value $MinimumFreeDiskPercent -Minimum 1 -Maximum 90
    ...
    if ($mode -eq 'Audit') {
        ...
        Write-Ok 'No findings: the event logs are configured to survive.'
        return 0
    }
    ...
    try {
        ...
        if ($mode -eq 'Apply') {
            ...
            Write-Info 'Retention takes effect on the next policy refresh; a reboot makes it immediate.'
            if ($script:Findings.Count -gt 0) {
        ...
        for ($i = $target.Changes.Count - 1; $i -ge 0; $i--) {
            ...
            try {
                ...
                if ($record.change.type -eq 'channelaccess') {
                    $outcome = Restore-ChannelAccess -ChangeRecord $record
                }
                else {
                    $outcome = Restore-TrackedChange -ChangeRecord $record
                }
