...
function Test-PSv2CanRun {
    <#
        ...
        3.5). Without it, 'powershell -Version 2' exits non-zero with "Version
        v2.0.50727 of the .NET Framework is not installed" - measured on the
        lab, exit code -65536 - even while DISM reports the feature Enabled.

        Only the exit code is trusted. The child process writes that message on
        a UTF-16 stream that comes back mojibake through a captured pipe, so
        matching on its text would be fragile in exactly the way this project
        avoids.
    #>
    ...
    try {
        # -Command exit 0 is the smallest possible successful v2 program.
        $null = & $engine -Version 2 -NoProfile -NonInteractive -Command 'exit 0' 2>&1
        $code = $LASTEXITCODE
    }
    catch {
        $ErrorActionPreference = $previous
        return [PSCustomObject] @{ CanRun = $false; Detail = ('launch threw: ' + $_.Exception.Message) }
    }
    finally {
        $ErrorActionPreference = $previous
    }
    ...
    if ($code -eq 0) {
        return [PSCustomObject] @{ CanRun = $true; Detail = 'powershell.exe -Version 2 ran successfully' }
    }
    return [PSCustomObject] @{
        CanRun = $false
        Detail = ('powershell.exe -Version 2 exited ' + $code + ' (engine unavailable)')
    }
}
