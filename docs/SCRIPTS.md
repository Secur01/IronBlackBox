...
- **Client SKU behaviour.** The lab is Windows Server, and Windows 10/11
  licensing is not available on EC2. Anything whose behaviour differs by
  `ProductType` — PSv2 feature names, Prefetch keys — is unproven on client
  SKUs and says so in its row.
